...
namespace ReleaseTools
{
	public class Program
	{
		...
		private static void UpdateExtensionManifest(string pathToSolution, ChangelogEntry changeEntry)
		{
			...
			extensionYamlUpdater.Update(Path.Combine(pathToSolution, $@"{AddonName}\extension.yaml"), changeEntry.Version);
		}

		private static string Build(string msBuild, string pathToSolution)
		{
			RunCommand(msBuild, $"{Path.Combine(pathToSolution, $"{AddonName}.sln")} -property:Configuration=Release");
			return Path.Combine(pathToSolution, AddonName, "bin", "Release");
		}
		...
		private static void UpdateInstallerManifest(string pathToSolution, ExtensionPackageNameGuesser extensionPackageNameGuesser, ChangelogEntry changeEntry)
		{
			...
			var playniteSdkVersionParser = new PlayniteSdkVersionParser(Path.Combine(pathToSolution, $@"{AddonName}\{AddonName}.csproj"));
